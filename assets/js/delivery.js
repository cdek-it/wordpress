(function($) {
    $(document).ready(function() {
        ...
        if ($('#woocommerce_official_cdek_seller_name').length === 0) {
            $('#woocommerce_official_cdek_client_secret')
              .after(
                '<p>Ошибка авторизации. Введите корректные идентификатор клиента и секретный ключ.</p>');
        } else {
            modeToggle();
            cityAutocomplete();
                ...
                $('#woocommerce_official_cdek_city')
                  .on('input', function(event) {
                      ...
                      if (event.target.value.length > 2) {
                          $.ajax({
                              method: 'GET',
                              url: window.cdek_rest_delivery_api_path.get_region,
                              data: {
                                      ...
                                      for (let i = 0; i < regions.length; i++) {
                                          if (regions[i].region ===
                                            regions[i].city) {
                                              $('#region-list')
                                                .append(
                                                  `<div class="reg-elem" data-code="${regions[i].code}" data-lat="${regions[i].latitude}" data-lon="${regions[i].longitude}">${regions[i].country}, ${regions[i].city}</div>`);
                                          } else {
                                              $('#region-list')
                                                .append(
                                                  `<div class="reg-elem" data-code="${regions[i].code}" data-lat="${regions[i].latitude}" data-lon="${regions[i].longitude}">${regions[i].country}, ${regions[i].region}, ${regions[i].city}</div>`);
                                          }
                                      }
                                  }
                              },
            ...
            function regionListProcess() {
                $('#region-list').on('click', function(event) {
                    ...
                    $('#woocommerce_official_cdek_city_code_value')
                      .val(event.target.getAttribute('data-code'));
                    $('#region-list').empty();
                    getPvz();
                });
            }
            ...
            function initMap() {
                $('#woocommerce_official_cdek_map')
                  .after(
                    '<div id="map-container"><div id="cdek-map"></div></div>');
                ...
                if (mapContainer.length) {
                    ...
                    if ($('#woocommerce_official_cdek_map_layer').val() ===
                      '1' && $('#woocommerce_official_cdek_yandex_map_api_key')
                        .val() !== '') {
                        L.yandex().addTo(map);
                    } else {
                        ...
                        if ($('#woocommerce_official_cdek_map_layer').val() ===
                          '1') {
                            $('#message')
                              .after(
                                '<div class="error"><p>Не удалось получить доступ к YandexMap по указанному ApiKey</p></div>');
                        }
            ...
            function modeToggle() {
                ...
                $('#woocommerce_official_cdek_mode').change(function(event) {
                    if ($(event.target).val() === '1') {
                        $($('label[for=woocommerce_official_cdek_address]')[0])
                          .text('Карта');
                        $('#map-container').show();
            ...
            function getPvz() {
                let cityCode = $('#woocommerce_official_cdek_city_code_value')
                  .val();
                if (cityCode !== cityCodePvzReceived) {
                    $.ajax({
                        method: 'GET',
                        url: window.cdek_rest_delivery_api_path.get_pvz,
                        data: {
            ...
            $('#woocommerce_official_cdek_tariff_list').change(function(event) {
                let tariffCode62 = $(event.currentTarget)
                  .find('option[value=62]');
                if ($(tariffCode62).is(':selected')) {
                    $('#woocommerce_official_cdek_service_list')
                      .find('option[value=DELIV_RECEIVER]')
                      .css('display', '');
                } else {
                    $('#woocommerce_official_cdek_service_list')
                      .find('option[value=DELIV_RECEIVER]')
            ...
            $('#woocommerce_official_cdek_service_list')
              .after(
                '<div id="service-message">Услуг для выбранных тарифов не найдено</div>');
            ...
            function checkServicesAvailable() {
                ...
                $('#woocommerce_official_cdek_tariff_list')
        ...
        $('#delete-order-btn').click(function(event) {
            ...
            $.ajax({
                method: 'GET',
                url: window.cdek_rest_delivery_api_path.delete_order,
                data: {
        ...
        $('#cdek-courier-delete').click(function(event) {
            $.ajax({
                method: 'GET',
                url: window.cdek_rest_delivery_api_path.call_courier_delete,
                data: {
                    order_id: $('input[name=package_order_id]').val(),
                },
                beforeSend: function() {
