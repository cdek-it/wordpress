/* eslint-disable no-console, no-alert, no-shadow, no-undef, @wordpress/no-unused-vars-before-return -- legacy debt, not part of this change */
'use strict';
...
$( document ).ready( () => {
	...
	const metaBox = $( '#official_cdek_order' )
		.on( 'change', '.create select', ( e ) =>
			metaBox
				.find( `.item[data-id=${ e.target.value }]` )
		...
		.on( 'click', '.create button.package', ( e ) => {
			...
			const packageData = {
				...
				height: parseInt( hei ),
				items: metaBox
					.find( `.item[data-id][aria-hidden=false]` )
					.map( ( i, e ) => ( {
						id: parseInt( e.dataset.id ),
						name: $( e ).text(),
						qty: parseInt(
							$( e ).find( 'input[type=number]' ).val()
						),
					} ) )
					.toArray(),
			};
			...
			if ( packageData.length < 1 ) {
				alert(
					__( 'Package length should be greater 1', 'cdekdelivery' )
				);
				return;
			}
			if ( packageData.width < 1 ) {
				alert(
					__( 'Package width should be greater 1', 'cdekdelivery' )
				);
				return;
			}
			if ( packageData.height < 1 ) {
				alert(
					__( 'Package height should be greater 1', 'cdekdelivery' )
				);
				return;
			}
			...
			metaBox
				.find( `.create .item[data-id][aria-hidden=false]` )
				.each( ( i, e ) => {
					...
					metaBox
						.find(
							`.create select option[value=${ e.dataset.id }]`
						)
						.remove();
				} );
		...
		.on( 'click', '.create button', ( e ) => {
			if (
				! Object.prototype.hasOwnProperty.call( e.target.dataset, 'id' )
			) {
				return;
			}
			...
			apiFetch( {
				...
				.then( ( r ) => r.text() )
				.then( ( t ) => metaBox.find( '.inside' ).html( t ) )
				.catch( ( e ) => console.error( e ) )
				.finally( () => loader.attr( 'aria-disabled', 'true' ) );
		} )
		.on( 'click', '.deletion', ( e ) => {
		...
		.on( 'click', '.toggle', ( e ) => {
			...
			target.ariaExpanded =
				target.ariaExpanded === 'true' ? 'false' : 'true';
		} )
		.on( 'click', '.intake button', ( e ) => {
			...
			metaBox
				.find( '.intake input[data-required]' )
				.each(
					( i, e ) =>
						( e.ariaInvalid = e.value === '' ? 'true' : 'false' )
				);

			if (
				metaBox.find(
					'.intake input[data-required][aria-invalid=true]'
				).length > 0
			) {
				return;
			}
			...
			apiFetch( {
				...
				data: {
					...
					weight: parseInt(
						metaBox.find( 'input[name=weight]' ).val()
					),
